...
(defn parse-multiplications
  "Parse and calculate the sum of all valid multiplications from the input string."
  [input]
  (->> input
...
(defn remove-donts
  "Remove all parts of the input string that are excluded by `[:dont]` and `[:do]` rules."
  [input]
...
(defn calculate-part-one
  "Calculate the sum of all valid multiplications (no filtering)."
  ([input]
   (parse-multiplications input))
  ([]
   (calculate-part-one (slurp "resources/day3_input.txt"))))

(defn calculate-part-two
  "Calculate the sum of all valid multiplications after applying `[:do]` and `[:dont]` rules."
  ([input]
